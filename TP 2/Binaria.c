...
int pesquisaBinaria(Veiculo vetor[], int n, char* modeloBuscado) {
    int esq = 0, dir = n - 1;

    while (esq <= dir) {
        int meio = (esq + dir) / 2;
        int resp = strcmp(modeloBuscado, vetor[meio].modelo);
...
int main() {
    ...
    char modeloPesquisado[100];

    
    while (fgets(modeloPesquisado, sizeof(modeloPesquisado), stdin) != NULL) {
        removerQuebraLinha(modeloPesquisado);

        
        if (modeloPesquisado[0] == '\0') {
            continue;
        }

        if (strcmp(modeloPesquisado, "FIM") == 0) {
            break;
        }

        if (pesquisaBinaria(vetor, totalveiculos, modeloPesquisado)) {
            printf("SIM\n");
        } else {
            printf("NAO\n");
        }
