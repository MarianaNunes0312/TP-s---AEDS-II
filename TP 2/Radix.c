...
void countingSort(Veiculo vetor[], int n, int pos) {
    Veiculo *saida = (Veiculo*) malloc(n * sizeof(Veiculo));
    int count[10] = {0}; 

    for (int i = 0; i < n; i++) {
        int digito = (vetor[i].ano / pos) % 10;
        count[digito]++;
    }
...
void radixSort(Veiculo vetor[], int n) {
    if (n <= 0) return;
