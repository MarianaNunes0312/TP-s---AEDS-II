...
class Veiculo {
    ...
    public String format() {
        ...
        String consumoC = String.format("%.2f", consumoCidade).replace(',', '.');
        String consumoE = String.format("%.2f", consumoEstrada).replace(',', '.');
...
class Insercao {
    ...
    public static void insertionSort(Veiculo[] vet, int n) {
        for (int i = 1; i < n; i++) {
            ...
            while ((j >= 0) && (vet[j].getMarca().compareTo(tmp.getMarca()) > 0)) {
                vet[j + 1] = vet[j];
                j--;
            }
            vet[j + 1] = tmp;
    ...
    public static void main(String[] args) throws Exception {
        ...
        insertionSort(pesquisados, totalPesquisados);
